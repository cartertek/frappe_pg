...
def check_patches_status():
	...
	status = {
		...
		"db_functions": {"installed": _check_db_functions_installed()},
	}
	...
	try:
		...
		status["trends_patch"] = {
			"applied": hasattr(trends, "based_wise_columns_query")
			and "patched_based_wise_columns_query" in str(trends.based_wise_columns_query)
		}
	except ImportError:
		status["trends_patch"] = {"applied": False, "reason": "ERPNext not installed"}
...
def reinstall_patches():
	...
	try:
		...
		return {"success": True, "message": "All patches reinstalled successfully"}

	except Exception as e:
		frappe.log_error(title="Patch Reinstallation Failed", message=str(e))
		return {"success": False, "message": f"Error reinstalling patches: {e!s}"}
...
def get_patch_info():
	...
	return {"app_version": __version__, "patches": patches, "total_patches": len(patches)}
